 ...
 * @LastEditors: berg yu
 * @LastEditTime: 2023-08-29 17:47:27
 * @Description: 请填写简介
-->
<template>
  <div class="container">
    <div class="robot-record__header">
      <div class="robot-record__query">
        医院名称：
        <el-input v-model="queryParams.hospitalName" placeholder="请输入" />
      </div>
      <div class="robot-record__query">
        状态：
  ...
  <el-dialog v-model="patchVisible" @close="resetForm(ruleFormRef)" title="医院新增or编辑" top="10vh">
    <el-form ref="ruleFormRef" :inline="false" :model="addParams" :rules="rules" label-width="120px" class="demo-ruleForm"
      status-icon>
      <el-form-item label="医院名称" prop="hospitalName">
        <el-input v-model="addParams.hospitalName" placeholder="请输入" />
      </el-form-item>
  ...
  ElMessageBox({
    ...
    beforeClose: (action, instance, done) => {
      if (action === 'confirm') {
        ...
        service
          .post('removeHospital', JSON.stringify([row.hospitalId]), {
            headers: {
      ...
      service.post('hospitalChange', param).then(
        (res: Object) => {
          if (res['success']) {
            //@ts-ignore
            ElMessage({
              message: param.hospitalId ? '编辑成功' : '新增成功',
              type: 'success'
            })
            query()
          } else {
            //@ts-ignore
            ElMessage({
              message: param.hospitalId ? '编辑失败' : '新增失败',
              type: 'error'
            })
          }
        },
